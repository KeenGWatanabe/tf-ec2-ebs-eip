...
resource "aws_eip" "rger_web_eip" {
  count    = var.settings.web_app.count #Creates multiple EIPs based on the count
  instance = aws_instance.rger_web[count.index].id #Associates the EIP with the EC2 instance

  tags = {
    Name = "rger_web_eip_${count.index}" #Tags the EIP with a unique name 
  }
}
